...
- **no-mistakes runs as its own `work`-job step, never inside the agent's turn.** The agent only implements, screenshots, and commits; a following step drives `no-mistakes axi run --yes` to a terminal outcome. The agent's `Bash` tool hard-caps at 10 min and auto-backgrounds longer calls, which silently kills the pipeline in headless `claude -p` - so the full review/test/CI run must live in a plain `run:` step bounded only by `timeout-minutes`. The agent's DB (Postgres `service`) and intent handoff (`INTENT_FILE`) exist to support this split.
